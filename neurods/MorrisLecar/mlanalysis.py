...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 18 21:24:09 2024
...
def makeTimeList(ti:float=0, tf:float=400, dt:float=0.1) -> npt.NDArray[float]:
    """
...
def solveML(system='single', solver='euler', Vrest=0,
            I0=0, Is=0, fs=0, ti=0, tf=400, dt=0.1, **kwargs):
    """
    Solves the Morris-Lecar (ML) system using solver. 

    Parameters
    ----------
    system : str, default is 'single'
        Type of ML system to solve.
...
def channelAsymptotes(Vmin=-100, Vmax=100, numpoints=400):
    """
    Solves the asymptotic values of (m, n) as t approaches infinity, given
    ...
    return (Phispace, inf_n(Phispace))
...
def timeConstants(Vmin=-100, Vmax=100, numpoints=400):
    ...
    """
    Phispace = np.linspace(Vmin, Vmax, numpoints)
    return (Phispace, tau_n(Phispace))

def firingRate(V, t, Vthresh=19):
    """
    Solves the firing rate or interspike interval (ISI) of an ML neuron.
